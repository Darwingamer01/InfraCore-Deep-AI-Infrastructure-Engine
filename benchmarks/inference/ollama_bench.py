#!/usr/bin/env python
"""
INFRACORE — Ollama Inference Benchmark Harness

Purpose: Measure throughput, latency, and TTFT for Ollama backend
...
class OllamaBench:
    ...
    async def generate(self, prompt: str) -> tuple[float, int, bool, str]:
        ...
        try:
            ...
            # For non-streaming, TTFT ≈ total latency / 2
            total_latency = (time.perf_counter() - start_time) * 1000
            ttft_ms = total_latency * 0.3  # Estimate TTFT as 30% of total
    ...
    async def run(self) -> BenchResult:
        ...
        print(f"\n{'='*70}")
        print(f"Ollama Benchmark: {config_name}")
...
async def main():
    ...
    parser.add_argument("--model", type=str, default="llama3.2:1b", help="Ollama model")
    ...
    try:
        ...
        output_file = output_dir / f"ollama_bench_c{args.concurrency}_{timestamp}.json"
